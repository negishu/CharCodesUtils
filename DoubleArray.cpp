...
BaseDoubleArray::BaseDoubleArray(void):_progress_(0),_array_(0),_used_(0),_alloc_size_(0),_values_(0),_Code_LowRange_(0),_Code_HighRange_(255)
{
}
BaseDoubleArray::~BaseDoubleArray(void)
{
    delete [] _used_;
    delete [] _array_;
}
unsigned int BaseDoubleArray::Resize(unsigned int new_size)
{
    unit_ tmp = {0, 0};
...
unsigned int BaseDoubleArray::Insert(const std::vector <node_> &siblings)
{
    ...
    next:
        ...
        for (size_t i = 1; i < siblings.size(); ++i) {
            if (_array_[begin + siblings[i].code].check != 0) goto next;
        }
        break;
    }
...
bool DoubleArray::Build(const char ** keys, const int* values, unsigned int size)
{
    _keys_   = keys;
    _values_ = values;
    _size_   = size;
    ...
    if (Fetch(root_node, siblings) == 0) return false;
    Insert(siblings);
    return true;
}    
unsigned int DoubleArray::Fetch(const node_ &parent, std::vector <node_> &siblings)
{
    unsigned int prev_code = 0;
    for (size_t i = parent.left; i < parent.right; ++i) {
        size_t len = strlen(_keys_[i]);
        if (len < parent.depth) continue;
        const unsigned char *tmp = (const unsigned char*)_keys_[i];
        unsigned int cur_code = 0;
        if (len != parent.depth) {
            cur_code = GetCurCode(tmp[parent.depth]) + 1;
        }
